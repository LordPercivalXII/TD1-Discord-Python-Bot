...
from MasterApprenticeLib.TD1_Lib_MasterApprentice_Control import MasterApprenticeLogVersionType, \
    apprentice_version_type, MAIN_DIR, delete_old_apprentice_log, FILENAME, AUTHOR
...
def get_log_dir():
    ...
    log_name = f"{FILENAME}_Apprentice_Log.log"
...
def get_new_log_dir():
    ...
    if delete_old_apprentice_log:
        ...
        if os.path.exists(new_log_dir):
            ...
            delete_old_logs(MAIN_DIR, f"{FILENAME}_Apprentice_Log [")

        return new_log_dir

    else:

        new_log_name = f"{FILENAME}_Apprentice_Log [{get_last_modified_time(MAIN_DIR, f'{FILENAME}_Apprentice_Log.log')}].log"
    ...
    def __init__(self, module_name, main_owner=None, additional_context=None, enable=True):
        ...
        if HAS_INIT is False and self.enable is True:
            ...
            with open(get_log_dir(), "w") as log_file:
                ...
                log_file.write(f"{AUTHOR}'s Apprentice Log - {FILENAME}")
                log_file.write("\nApprentice Version: {0}".format(
                    MasterApprenticeLib.TD1_Lib_MasterApprentice_Control.__version__))
                log_file.write(
                    "\n==================================================================================================")
                log_file.write(
                ...
                log_file.write(
                    "\n==================================================================================================")
                if apprentice_version_type != MasterApprenticeLogVersionType.DEVELOPER:
